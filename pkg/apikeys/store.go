...
//
// Used by the inbound auth chain (pkg/auth/inbound) when api_keys.db.enabled
// is true. File-based keys live in pkg/auth/inbound directly; this package
// is only needed for keys that operators rotate via the portal.
package apikeys
...
// Key is the persistent record. Hash is never returned by List (omitted from
// JSON), and the plaintext value only exists at creation time.
type Key struct {
	ID          string     `json:"id"`
...
// Create mints a new API key with a random plaintext value, hashes it
// with bcrypt, and inserts a row. The plaintext is returned to the
// caller exactly once.
//
// `name` must be unique. If a row with the same name exists, the call
// returns an error.
...
// Authenticate scans every (non-expired) row, comparing the candidate
// against each stored bcrypt hash. Bcrypt is intentionally slow so this
// scales poorly past ~thousands of keys; fine for a test fixture.
//
// On match, last_used_at is bumped (best-effort).
...
// recognizability ("at_" = api-test).
func generatePlaintext() (string, error) {
	buf := make([]byte, 32)
	if _, err := rand.Read(buf); err != nil {
		return "", err
	}
	return "at_" + strings.TrimRight(base64.URLEncoding.EncodeToString(buf), "="), nil
}
