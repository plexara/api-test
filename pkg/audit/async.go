...
// drained by a background worker. The synchronous request path enqueues
// in O(1); the goroutine handles the actual database write.
//
// On a full buffer the event is dropped (and counted) so the audit
// pipeline can never block a request. Operators preferring lossless audit
// must size the buffer for their peak rate.
...
// NewAsyncLogger returns a buffered async wrapper around inner.
//
// bufferSize is the channel depth; perCallTimeout bounds each underlying
// Log call. Call Close() during shutdown to drain the queue.
...
func (a *AsyncLogger) write(ev Event) {
	...
	if err := a.inner.Log(ctx, ev); err != nil {
		a.logger.Warn("audit write failed", "method", ev.Method, "path", ev.Path, "err", err)
	}
}
...
func (NoopLogger) Query(context.Context, QueryFilter) ([]Event, error) { return nil, nil }
