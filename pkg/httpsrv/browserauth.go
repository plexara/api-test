...
// used elsewhere by the server (typically the same instance shared with
// inbound JWT validation). The PKCE signing key is derived from the
// long-lived cookie secret with a domain separator so a leak of one
// cookie does not directly forge the other.
func NewBrowserAuth(
	ctx context.Context,
	cfg *config.Config,
	validator *auth.OIDCAuthenticator,
	...
	pkceStore, err := NewSessionStore("api_test_pkce", pkceSecret, cfg.Portal.CookieSecure, 0)
	if err != nil {
		return nil, err
	}
...
func derivePKCESecret(cookieSecret string) string {
	...
	_, _ = mac.Write([]byte("api-test/pkce/v1"))
...
func (b *BrowserAuth) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	http.SetCookie(w, &http.Cookie{ // nosemgrep: go.lang.security.audit.net.cookie-missing-secure.cookie-missing-secure -- Secure follows config; dev/HTTP loopback uses false intentionally.
...
func (b *BrowserAuth) handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	// nosemgrep: go.lang.security.injection.open-redirect.open-redirect -- returnTo is constrained by sanitizeReturnPath to a same-origin path; cross-origin and protocol-relative forms are rejected at login time.
	http.Redirect(w, r, returnTo, http.StatusFound)
}

func (b *BrowserAuth) handleLogout(w http.ResponseWriter, _ *http.Request) {
	b.sessions.Clear(w)
...
func (b *BrowserAuth) exchangeCode(ctx context.Context, code, verifier string) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		// Truncate IdP body to a status + length, never embed the raw body in
