...
// ProtectedResourceMetadata responds to RFC 9728 metadata queries. The api-test
// server is the protected resource; the configured OIDC issuer is the
...
// base URL so it stays stable regardless of where REST handlers are mounted.
...
func AuthorizationServerStub(cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, _ *http.Request) {
		...
		body := map[string]any{
			...
			"note":                     "api-test delegates auth to the issuer above; fetch the openid_configuration_url for the real authorization server metadata",
		}
		_ = json.NewEncoder(w).Encode(body)
	}
}
