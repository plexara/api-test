...
# meaningfully. cmd/api-test is a tiny entry point that's covered by the
...
#
# CI runs the integration suite (.github/workflows/ci.yml `integration` job)
# separately against testcontainers Postgres, at which point those
# packages are also exercised.
#
# Usage:
...
EXCLUDE_PACKAGES=(
  "github.com/plexara/api-test/cmd/"
  "github.com/plexara/api-test/pkg/apikeys/"
  "github.com/plexara/api-test/pkg/audit/postgres/"
...
go tool cover -func="$FILTERED" | awk '
  /^total:/ { next }
  {
    sub(/^github.com\/plexara\/api-test\//, "", $1)
    split($1, p, "/")
    pkg = ""
